...
export default class GradCursorControl implements MapboxIControl {
	...
	onAdd(map: MapboxMap): HTMLElement {
		...
		this._mapMouseDownCallback = (): void => {
			if (this._context) this._context.strokeStyle = '#CC9900';
		};
		this._mapMouseUpCallback = (): void => {
			if (this._context) this._context.strokeStyle = '#B21A00';
		};
	...
	onRemove(): void {
		...
		if (this._mapMouseEnterCallback !== undefined)
			this._map.getCanvas().removeEventListener('mouseenter', this._mapMouseEnterCallback);
		if (this._mapMouseLeaveCallback !== undefined)
			this._map.getCanvas().removeEventListener('mouseleave', this._mapMouseLeaveCallback);
		if (this._mapMouseMoveCallback !== undefined)
			this._map.getCanvas().removeEventListener('mousemove', this._mapMouseMoveCallback);
	...
	private redraw(): void {
		if (this._context === null || this._canvas === null || this._map === null || this._currentPosition === null)
			return;
